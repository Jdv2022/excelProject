...
export default function HeatMapTool({updateData}){
    ...
    useEffect(()=>{
        ...
        function operation(obj){SVGAnimatedBoolean
                ...
                <>
                    {orig.map((item, i) => (
                        <div className='heatmap2 mb-1' key={i}>
                            ...
                            <input type="number" className="heatmap2" placeholder='NaN' key={`${item.x}${item.y}${item.val}`} defaultValue={item.val} onChange={(e)=>handleEditedData({data0:`${item.y}${item.x}`, data1:e.target.value})} />
                        </div>
                    ))}
                </>
            )
            function handleEditedData(params){
